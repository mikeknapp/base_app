...
export function TableLoading(props: { border?: boolean } = { border: true }) {
  return <div role="status" className={`${props.border ? "border border-gray-200" : ""} w-full mt-5 p-4 space-y-6 divide-y divide-gray-200 rounded shadow animate-pulse dark:divide-gray-700 md:p-6 dark:border-gray-700`}>
    {[...Array(6)].map((_, i) => <div
      key={`table-loading-${i}`} className={`flex items-center justify-between ${i > 0 ? "pt-4" : ""}`}>
      <div>
